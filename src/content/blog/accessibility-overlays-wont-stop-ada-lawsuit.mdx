...
eLan Technology provides accessibility-first development, scoped WCAG audits, source-level remediation and retesting for clients in India and overseas. We do not present an overlay as a substitute for correcting and verifying the underlying website.

Our accessibility engagements can combine automated and manual testing, with findings prioritised by user impact, repeated source component and engineering effort. Evidence, retesting, an Accessibility Conformance Report using an appropriate VPAT template, or statement support are included only when named in the scope. Legal counsel should decide how technical evidence is used in a claim, filing or procurement response.
